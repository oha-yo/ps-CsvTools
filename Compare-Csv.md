...
- ストリーミング読みを使っているので大量データ（数十万レコード）を扱っても メモリ消費が安定している
- EPPlusを使っているのでエクセル本体がなくとも利用可能。（その代わり追加でライブラリのダウンロードが必要）
- EPPlusを使っているのでCOM オブジェクト（Excel.Application）と比較して圧倒的に高速
